 ...
 */
export const ForeignHtmlRenderer = function (styleSheets) {
  const self = this;
  ...
  const parseValue = function (str, startIndex, prefixToken, suffixTokens) {
    //console.log(str, startIndex, prefixToken, suffixTokens);
  ...
  const buildSvg = async function (contentHtml, width, height) {
    // copy styles
    let cssStyles = '';
    let urlsFoundInCss = [];
    ...
    const fetchedResourcesFromStylesheets =
      await getMultipleResourcesAsBase64(urlsFoundInCss);
    for (let i = 0; i < fetchedResourcesFromStylesheets.length; i++) {
      ...
      cssStyles = cssStyles.replace(
        new RegExp(escapeRegExp(r.resourceUrl), 'g'),
        r.resourceBase64,
      );
    }
    ...
    const fetchedResources =
      await getMultipleResourcesAsBase64(urlsFoundInHtml);
    ...
    const contentRootElemString = new XMLSerializer().serializeToString(
      contentRootElem,
    );
  ...
  this.renderToSVG = async function (html, width, height) {
    return await buildSvg(html, width, height);
  };
